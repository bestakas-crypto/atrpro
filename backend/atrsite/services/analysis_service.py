...
금리/일본 국채금리/JPY환율 등)은 LLM 웹서치로 찾되, "객관적 조회"와
"해석/판단"을 역할을 나눈 2단계 구조를 쓴다:

  1단계(웹서치 켬, chain="stage1_search"): SOXX/MU/US10Y/US30Y/JP10Y/
    JPY-KRW 등을 있는 그대로 찾아 보고만 함 -- 해석/추천 없이 사실+출처만.
  2단계(chain="stage2_judgment"): 검증된 스냅샷 + VXN 그리드 판정(Python
    확정 결과) + 1단계 조회 결과를 근거로 최종 10항목 브리핑을 작성.

  실제 공급자 우선순위는 llm_client.py의 _FALLBACK_CHAINS 참고(2026-08-04:
  사용자가 제미나이를 신뢰하지 않아 -- 환각/과거데이터 버퍼 증상, 시황
  웹서치 괴리 -- 두 체인 모두 GPT를 1순위로 바꿈. 이전엔 제미나이 우선).
...
# ---------------------------------------------------------------------------
# 1단계: 객관적 조회(chain="stage1_search", 현재 GPT 우선) -- SOXX/MU는 사용자 확인 항목, DXY/VIX는
...
[말투 -- 반드시 지킬 것]
친한 언니가 재무/경제를 하나도 모르는 완전 초보 후배에게 설명해주는
말투로 쓰십시오. 친근하고 다정하게, "~해!", "~거든", "~야" 같은 구어체를
쓰고 이모지를 적절히 섞으십시오. 전문용어(VXN, 국채금리, 그리드 매수
등)가 나오면 괄호로 아주 쉬운 말로 짧게 풀어주십시오. 글 맨 앞에 짧고
밝은 인사를, 맨 끝에는 언니의 총평과 "더 궁금하면 물어봐~" 같은 마무리
인사를 붙이십시오.
**단, 이 브리핑은 운전 중 음성으로도 듣는 용도입니다** -- 한 문장이 너무
길어지거나 이모지 설명이 장황해지지 않게, 친근하되 문장은 짧고 명확하게
끊어 쓰십시오(듣다가 무슨 말인지 놓치면 안 됨).

[숫자·판정 관련 절대 규칙 -- 말투를 쉽게 바꾸는 것과 무관하게 반드시 지킬 것]
1. "검증된 Macro Snapshot"에 제공된 수치와 "1단계 객관적 조회 결과"에
   있는 내용만 근거로 삼으십시오. 그 외의 수치(금리, 지수, 환율 등)를
   스스로 알고 있다고 가정해서 말하면 안 됩니다. 모르는 값은 "확인 안
   됨"이라고 쉽게 풀어서라도 반드시 언급하십시오(그냥 빼먹지 마십시오).
2. "VXN 그리드 규칙 판정"은 Python 프로그램이 이미 확정한 결과입니다.
   말투는 쉽게 풀어써도 되지만, 재계산하거나 다르게 판단하지 말고 그
   결과를 정확히 그대로 전달하십시오.
3. 이 프로그램은 절대 자동으로 주문을 넣지 않습니다. 매수/매도를 단정적
   으로 지시하지 말고, "이건 참고만 하고 진짜 사고파는 건 네가 직접
   정해야 해!"처럼 참고용임을 분명히 하십시오.

[구조 -- 아래 10개 내용을 전부 다루되, 번호 대신 이모지+친근한 소제목을
붙이십시오]
   1) 한 줄 결론
